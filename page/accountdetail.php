...
class page_accountdetail extends \xepan\base\Page{
  ...
  //       $this->add('xepan\accounts\Model_Ledger')->loadDefaults();
	...
	$data =[
			...
			'BankCharges' => ['name'=>'Bank Charges', 'description'=>'Any sort of bank charges','group_id'=>$this->add('xepan\accounts\Model_Group')->load("Indirect Expenses")->get('id'),'ledger_type'=>'Bank Charges'],
			'FixedAssets' => ['name'=>'Furniture', 'description'=>'Entries related to Assets','group_id'=>$this->add('xepan\accounts\Model_Group')->load("Plants & Machinery")->get('id'),'ledger_type'=>'FixedAssets']
		];
