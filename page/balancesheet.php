...
class page_balancesheet extends \xepan\base\Page{
	...
	function init(){
		...
		$model_balancesheet = $this->add('xepan\accounts\Model_BalanceSheet');
		$model_balancesheet->addCondition('report_name','BalanceSheet');

		foreach ($model_balancesheet as $bs) {
			$model_ledger = $this->add('xepan\accounts\Model_Ledger');
			$model_ledger_j = $model_ledger->join('ledger.balance_sheet_id',$bs['id']);
			$model_ledger_j->addField('id');

			$ledger_transaction_j = $model_ledger_j->join('account_transaction_row'); 
		}
